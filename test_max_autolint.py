...
class TestAutoLint():
    ...
    def test_check(test_files):
        ...
        if test_type == "unit":
            ...
            autolinter.checkers=[MockFlake8Checker(), MockBlackChecker(), MockIsortChecker()]
    ...
    @pytest.mark.parametrize("check_first", (True, False), ids=["checker_first", "do_not_check_first"])
    def test_modify(self, test_files, check_first):
        ...
        if test_type == "unit":
            ...
            autolinter = max_autolint.MaxAutolint()
            autolinter.modifiers={MockBlackModifier(), MockIsortModifier()}
            autolinter.checkers={MockBlackChecker(), MockIsortChecker(), MockFlake8Checker()}
            autolinter.syntax=MockSyntax()
        ...
        for _, file in test_files.items():
            ...
            if file.syntax_error_present or len(file.checker_no_errors) < len(autolinter.checkers):
